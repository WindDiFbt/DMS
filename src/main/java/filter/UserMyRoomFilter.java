...
@Setter
@Getter
@WebFilter(filterName = "UserMyRoomFilter", urlPatterns = {"/*"})
public class UserMyRoomFilter implements Filter {

    private static final boolean debug = true;

    private FilterConfig filterConfig = null;

    public UserMyRoomFilter() {
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (debug) {
            log("UserMyRoomFilter:doFilter()");
        }

        doBeforeProcessing(request, response);

        String path = req.getRequestURI().substring(req.getContextPath().length());
        if (path.startsWith("/user/myroom")) {
            ...
            if (account != null) {
                ...
                if (information == null || information.getRoomName() == null || information.getRoomName().isEmpty()) {
